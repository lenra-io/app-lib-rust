...
pub struct Api {
    pub(self) client: &InternalApiClient,
...
impl Api {
    pub(crate) fn new(params: ApiParam) -> Api {
        ...
        let client = InternalApiClient::new(&params.url, &params.token);
        Api {
            client: &client,
            params: &params,
            data: DataApi {
...
impl ApiTrait for Api {
    ...
    fn client(&self) -> &InternalApiClient {
        self.client
    }
}
...
impl Collection {
    fn get_doc<T: Doc>(&self, id: &str) -> Result<T> {
        log::debug!("get_doc {}[{}]", self.coll, id);
        let response = self.client.get_document_by_id(self.coll.as_str(), id).await?;
...
impl CollectionTrait for DataApi {}
...
impl CollectionTrait for Transaction {}
